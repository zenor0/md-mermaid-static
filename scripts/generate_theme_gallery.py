...
"""
    ...
    for theme_name, theme_files in sorted(themes.items()):
        ...
        if not flowchart_preview.exists():
            print(f"警告: {theme_name} 主题没有预览图像，请先运行 generate_theme_previews.py")
            continue
        ...
        if 'config' in theme_files:
            try:
                with open(theme_files['config'], 'r', encoding='utf-8') as f:
                    ...
                    theme_info = f"基于 {base_theme} 主题{'，深色模式' if dark_mode else ''}"
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=Path('themes/gallery.html'),
